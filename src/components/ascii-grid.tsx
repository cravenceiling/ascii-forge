...
import type { SparseGrid, ViewState, Point } from "@/types";
import { CHAR_WIDTH, CHAR_HEIGHT, ASCII_CHARS } from "@/types";
...
export const AsciiGrid = memo(forwardRef<AsciiGridHandle, AsciiGridProps>(function AsciiGrid({
  committedGrid,
  scratchGrid,
  viewState,
  showGrid,
  ...
  useImperativeHandle(ref, () => ({
    screenToGrid: (screenX: number, screenY: number) => {
      ...
      const relativeX = (screenX - rect.left - viewState.offsetX) / viewState.zoom;
      const relativeY = (screenY - rect.top - viewState.offsetY) / viewState.zoom;
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      const endCol = Math.ceil((canvas.width - viewState.offsetX) / (CHAR_WIDTH * viewState.zoom)) + padding;
      const startRow = Math.max(0, Math.floor(-viewState.offsetY / (CHAR_HEIGHT * viewState.zoom)) - padding);
      const endRow = Math.ceil((canvas.height - viewState.offsetY) / (CHAR_HEIGHT * viewState.zoom)) + padding;
